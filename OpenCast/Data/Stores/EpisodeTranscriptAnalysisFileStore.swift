...
struct EpisodeTranscriptAnalysisFileStore: AnalysisDocumentFileStore {
    nonisolated static let directoryName = "EpisodeTranscriptAnalyses"
    ...
    nonisolated func read(relativePath: String) throws -> EpisodeTranscriptAnalysisDocument {
        let data = try Data(contentsOf: fileURL(relativePath: relativePath))
        return try EpisodeTranscriptAnalysisJSONCoding.decoder()
            .decode(EpisodeTranscriptAnalysisDocument.self, from: data)
    }

    @concurrent
    func readOffCaller(relativePath: String) async throws -> EpisodeTranscriptAnalysisDocument {
        try read(relativePath: relativePath)
    }

    nonisolated func write(_ document: EpisodeTranscriptAnalysisDocument, relativePath: String) throws {
        ...
        let data = try JSONEncoder.openCastTranscriptAnalysisEncoder.encode(document)
...
extension EpisodeTranscriptAnalysisFileStore {
...
private extension JSONEncoder {
    nonisolated static var openCastTranscriptAnalysisEncoder: JSONEncoder {
        EpisodeTranscriptAnalysisJSONCoding.encoder(outputFormatting: [.prettyPrinted, .sortedKeys])
    }
}
