...
nonisolated struct AppAttestKeychain: Sendable {
    ...
    func loadOrCreateInstallID() throws -> String {
        if let installID = try loadInstallID() {
            return installID
        }
    ...
    private func loadString(account: String) throws -> String? {
        ...
        guard status == errSecSuccess else {
            throw AppAttestKeychainError(status: status)
        }

        guard let data = result as? Data,
              let value = String(data: data, encoding: .utf8)
        else {
            throw AppAttestKeychainError(status: errSecDecode)
        }
    ...
    private func saveString(_ value: String, account: String) throws {
        ...
        guard updateStatus == errSecItemNotFound else {
            throw AppAttestKeychainError(status: updateStatus)
        }
        ...
        guard addStatus == errSecSuccess else {
            throw AppAttestKeychainError(status: addStatus)
        }
    }
