...
Use your own Apple development team and unique bundle identifiers for the app, both notification extensions, and the `OpenCastWidgets` extension. Update the iCloud container to one your team controls and enable Siri for the App ID and provisioning profile. Create an App Group owned by your team and enable it for both the app and widget. Replace `group.com.connor.opencast` in the three app entitlement files under `OpenCast/Resources/`, in `OpenCastWidgets/OpenCastWidgets.entitlements`, and in `ResumeWidgetSnapshot.appGroup` (`OpenCast/Widgets/ResumeWidgetSnapshot.swift`) with the same group identifier. CarPlay audio is granted by Apple per team; remove `com.apple.developer.carplay-audio` from the selected entitlements file if your team does not have it. Private Cloud Compute is granted per team the same way and backs transcript Recap and Ask, so remove `com.apple.developer.private-cloud-compute` as well if your team does not have it. Without that capability — or on a device or build without Apple Intelligence — Recap and Ask report that they are unavailable, and the rest of the app is unaffected.
