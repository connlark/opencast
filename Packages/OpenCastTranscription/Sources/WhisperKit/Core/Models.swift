...
/// OpenCast fork: lightweight once-per-window signal carrying the window
/// ordinal, so callers can track decode progress without paying for
/// per-token hypothesis callbacks.
public typealias WindowStartCallback = @Sendable (_ windowIndex: Int) -> Void
