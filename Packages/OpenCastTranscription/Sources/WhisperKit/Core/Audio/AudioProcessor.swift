...
public extension AudioProcessing {
    ...
    /// OpenCast fork: bounded window copy straight from the full audio array
    /// (see AudioProcessor.padOrTrimAudio(fromArray:startAt:availableLength:toLength:)).
    func padOrTrim(fromArray audioArray: [Float], startAt startIndex: Int, availableLength: Int, toLength frameLength: Int) -> (any AudioProcessorOutputType)? {
        AudioProcessor.padOrTrimAudio(fromArray: audioArray, startAt: startIndex, availableLength: availableLength, toLength: frameLength)
    }
    ...
    /// OpenCast fork: window copy without the intermediate `[Float]` slice.
    /// Copies directly from the complete audio array; `availableLength` bounds
    /// the copy so a clip end earlier than the array end is respected.
    static func padOrTrimAudio(
        fromArray audioArray: [Float],
        startAt startIndex: Int,
        availableLength: Int,
        ...
        guard startIndex >= 0, startIndex < audioArray.count, availableLength > 0 else {
            Logging.error("Audio window is outside the buffer size")
            return nil
        }

        let endFrameIndex = min(audioArray.count, startIndex + min(availableLength, frameLength))
        let actualFrameLength = endFrameIndex - startIndex
        ...
        if actualFrameLength > 0 {
            audioArray.withUnsafeBufferPointer { sourcePointer in
                audioSamplesArray.dataPointer.assumingMemoryBound(to: Float.self).initialize(from: sourcePointer.baseAddress!.advanced(by: startIndex), count: actualFrameLength)
            }
        }
...
open class AudioProcessor: NSObject, AudioProcessing {
    ...
    // OpenCast fork (whisper-perf E2): single direct copy. Upstream built the
    // array from 1,024-frame temporaries and zeroed the source buffer as it
    // went; every caller discards the buffer immediately after conversion, so
    // the zeroing bought nothing and the temporaries were pure churn.
    public static func convertBufferToArray(buffer: AVAudioPCMBuffer) -> [Float] {
        guard let channelData = buffer.floatChannelData else {
            return []
        }
        return [Float](UnsafeBufferPointer(start: channelData[0], count: Int(buffer.frameLength)))
    }
