...
final class OpenCastUITests: XCTestCase {
    ...
    @MainActor
    func testOptInExampleDeviceE2EShowRemoteFeedCanPlayFromAppUI() throws {
        ...
        guard shouldRunRemoteProbe else {
            throw XCTSkip("Set OPENCAST_RUN_REMOTE_VOICEBOOST_UI_TESTS=1 or create /tmp/opencast-run-remote-voiceboost-ui-tests to run the live Example Device E2E Show UI playback probe.")
        }
        ...
        app.launchEnvironment["OPENCAST_DEFAULT_FEED_URL"] = "https://debuggers-almanac.example.com/feed.xml"
        app.launchEnvironment["OPENCAST_CAPTURE_VOICEBOOST_DIAGNOSTICS"] = "1"
        app.launch()
        ...
        assertExists(app.staticTexts["Example Device E2E Show"], named: "Example Device E2E Show subscription", timeout: 30)
        openInbox(in: app)

        let firstEpisode = debuggerAlmanacFirstEpisode(in: app)
        assertExists(firstEpisode, named: "Example Device E2E Show inbox episode", timeout: 30)
        firstEpisode.tap()
    ...
    @MainActor
    func testOptInExampleDeviceE2EShowNowPlayingArtworkPeelScreenshot() throws {
        ...
        guard shouldRunRemotePeelProbe else {
            throw XCTSkip("Set OPENCAST_RUN_REMOTE_PEEL_UI_TESTS=1 or create /tmp/opencast-run-remote-peel-ui-tests to run the live Example Device E2E Show peel visual probe.")
        }
    ...
    private func verifyAddPodcastRSSClipboardPrefill(
        ...
        let pastedFeedURL = "http://seed.example.com/feed.xml"
        let app = makeSeededApp(
