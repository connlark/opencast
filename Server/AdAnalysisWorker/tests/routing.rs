...
fn health_returns_ok_without_auth() {
    let response = static_response(route_request("GET", HEALTH_PATH, false, false, false));
...
fn analysis_requires_post_enabled_and_auth() {
    ...
    assert!(matches!(
        route_request("POST", ANALYZE_TRANSCRIPT_PATH, true, false, false),
...
fn app_attest_setup_routes_require_post_and_enabled_feature() {
    ...
    assert!(matches!(
        route_request("POST", APP_ATTEST_CHALLENGE_PATH, true, false, false),
        RouteAction::AppAttestChallenge
    ));
    assert!(matches!(
        route_request("POST", APP_ATTEST_REGISTER_PATH, true, false, false),
...
fn unknown_routes_and_wrong_health_method_return_json_errors() {
    let not_found = static_response(route_request("GET", "/missing", false, false, false));
    assert_eq!(not_found.status, 404);
    assert_header(&not_found, "content-type", JSON_CONTENT_TYPE);
    assert_eq!(not_found.body, r#"{"error":"not_found"}"#);

    let health_post = static_response(route_request("POST", HEALTH_PATH, false, false, false));
...
fn poll_routes_require_post_enabled_and_a_valid_job_id() {
    ...
    let wrong_method = static_response(route_request("GET", &path, true, false, false));
    assert_eq!(wrong_method.status, 405);
    assert_header(&wrong_method, "allow", "POST");

    let disabled = static_response(route_request("POST", &path, false, false, false));
    assert_eq!(disabled.status, 503);
    assert_eq!(disabled.body, r#"{"error":"ad_analysis_disabled"}"#);

    assert_eq!(
        route_request("POST", &path, true, false, false),
        RouteAction::PollJob {
...
#[test]
fn internal_host_resolves_only_the_two_binding_paths() {
    let fingerprint = "f".repeat(64);
    let poll_path = format!("{INTERNAL_JOBS_PREFIX}{fingerprint}/poll");

    assert!(matches!(
        route_request("POST", INTERNAL_ANALYZE_PATH, false, true, true),
        RouteAction::InternalAnalyze
    ));
    assert_eq!(
        route_request("POST", &poll_path, false, true, true),
        RouteAction::InternalPollJob {
            job_id: fingerprint.clone()
        }
    );

    // The internal kill switch is independent of the public flag.
    let disabled = static_response(route_request(
        "POST",
    ...
    assert_eq!(disabled.status, 503);
    assert_eq!(disabled.body, r#"{"error":"ad_analysis_disabled"}"#);
    let poll_disabled = static_response(route_request("POST", &poll_path, true, true, false));
    assert_eq!(poll_disabled.status, 503);
    ...
    for path in [
        ...
        ANALYZE_TRANSCRIPT_PATH,
        APP_ATTEST_CHALLENGE_PATH,
        "/internal/v1/jobs/short/poll",
        "/internal/v1/jobs/missing-suffix",
        "/internal/v1/other",
    ] {
        let response = static_response(route_request("POST", path, true, true, true));
        assert_eq!(response.status, 404, "internal host path {path}");
    }
}

#[test]
fn public_host_never_serves_internal_paths() {
    let fingerprint = "f".repeat(64);
    for path in [
        INTERNAL_ANALYZE_PATH.to_string(),
        format!("{INTERNAL_JOBS_PREFIX}{fingerprint}/poll"),
    ] {
        // Even with every flag on, the public host 404s /internal/*.
        let response = static_response(route_request("POST", &path, true, false, true));
        assert_eq!(response.status, 404, "public host path {path}");
    }
}
