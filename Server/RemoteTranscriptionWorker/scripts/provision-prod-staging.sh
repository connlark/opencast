...
echo "== D1 migrations"
yarn wrangler d1 migrations apply "$GW_DB_NAME" --remote --env prod-staging
(cd "$PURCHASE_DIR" && yarn wrangler d1 migrations apply "$PW_DB_NAME" --remote --env prod-staging)

echo "== Gateway secrets (names only; set missing with 'yarn wrangler secret put <NAME> --env prod-staging')"
GW_SECRETS=$(yarn wrangler secret list --env prod-staging 2>/dev/null || echo "[]")
...
PW_SECRETS=$(cd "$PURCHASE_DIR" && yarn wrangler secret list --env prod-staging 2>/dev/null || echo "[]")
...
echo "== Dry runs"
yarn wrangler deploy --dry-run --outdir dist --env prod-staging >/dev/null && echo "  gateway prod-staging dry-run OK"
(cd "$PURCHASE_DIR" && yarn wrangler deploy --dry-run --outdir dist-prod-staging --env prod-staging >/dev/null && echo "  purchase prod-staging dry-run OK")
(cd "$MEDIA_DIR" && yarn wrangler deploy --dry-run --outdir dist -c wrangler.prod-staging.jsonc >/dev/null && echo "  media prod-staging dry-run OK")
