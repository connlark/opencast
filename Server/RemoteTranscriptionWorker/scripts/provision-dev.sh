...
echo "== D1: $DB_NAME"
if ! yarn wrangler d1 info "$DB_NAME" >/dev/null 2>&1; then
  yarn wrangler d1 create "$DB_NAME"
fi
DB_ID=$(yarn wrangler d1 info "$DB_NAME" --json 2>/dev/null | python3 -c 'import json,sys; print(json.load(sys.stdin)["uuid"])')
...
echo "== D1 migrations"
yarn wrangler d1 migrations apply "$DB_NAME" --remote
...
for name in DEV_BEARER_TOKEN URL_ENCRYPTION_KEY; do
  if yarn wrangler secret list 2>/dev/null | grep -q "\"$name\""; then
    echo "  $name: present"
  else
    echo "  $name: MISSING"
  fi
...
echo "== Dry runs"
yarn wrangler deploy --dry-run --outdir dist >/dev/null && echo "  gateway dry-run OK"
(cd "$MEDIA_DIR" && yarn wrangler deploy --dry-run --outdir dist >/dev/null && echo "  media dry-run OK")
