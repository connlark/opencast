...
use crate::billing::{
    authoritative_duration_seconds, billing_id_from_token, charge_seconds_for_duration,
    purchase_install_key, validate_billing_required, validate_credit_backend, validate_lane,
...
use crate::storage;
use crate::types::{
    resolve_gemini_model, BootstrapRequest, BootstrapResponse, ErrorResponse,
...
const TRANSCRIPT_ANALYSIS_CLIENT_TOKEN: &str = "TRANSCRIPT_ANALYSIS_CLIENT_TOKEN";
const PUBLIC_TRANSCRIPT_ANALYSIS_ENABLED: &str = "PUBLIC_TRANSCRIPT_ANALYSIS_ENABLED";
...
const LANE: &str = "LANE";
const BILLING_REQUIRED: &str = "BILLING_REQUIRED";
const CREDIT_BACKEND_VAR: &str = "CREDIT_BACKEND";
const DEV_CREDIT_GRANT_SECONDS: &str = "DEV_CREDIT_GRANT_SECONDS";
const DEFAULT_DEV_CREDIT_GRANT_SECONDS: i64 = 36_000;
...
// The AppTransaction JWS is capped at 32 KiB by PurchaseWorker; the envelope
// wraps it (base64-ish, no escaping blowup) plus install/key ids and the
// assertion.
const MAX_ACCOUNT_BOOTSTRAP_BODY_BYTES: usize = 64 * 1024;

pub async fn handle_request(mut req: Request, env: Env) -> Result<Response> {
    let method = req.method();
    let path = req.path();
    let enabled = env_flag(&env, PUBLIC_TRANSCRIPT_ANALYSIS_ENABLED, false);
    ...
    match route_request(method.as_ref(), &path, enabled) {
        ...
        RouteAction::AccountBootstrap => handle_account_bootstrap(&mut req, &env).await,
...
pub(crate) struct AppConfig {
    ...
    /// Deployment lane (development | prod-staging | production); the
    /// dev credit fake is forbidden outside development (fail closed).
    pub(crate) lane: String,
    /// Per-lane pay-gate kill switch: absent/false ships the
    /// billing code dark; true makes the App Attest lane bill and fail
    /// CLOSED when the credit backend is unreachable. The bearer probe lane
    /// never bills either way.
    pub(crate) billing_required: bool,
    pub(crate) credit_backend: CreditBackend,
    pub(crate) dev_credit_grant_seconds: i64,
}

impl AppConfig {
    pub(crate) fn from_env(env: &Env) -> std::result::Result<Self, ErrorResponse> {
        ...
        let dev_credit_grant_seconds = env
            .var(DEV_CREDIT_GRANT_SECONDS)
            .ok()
            .and_then(|value| value.to_string().trim().parse::<i64>().ok())
            .unwrap_or(DEFAULT_DEV_CREDIT_GRANT_SECONDS);
        ...
        Ok(Self {
            ...
            billing_required,
            credit_backend,
            dev_credit_grant_seconds,
        })
    }

    pub(crate) fn credit_authority(&self, env: &Env) -> Result<CreditAuthority> {
        CreditAuthority::from_env(
            env,
            self.credit_backend,
            &self.lane,
...
async fn analyze_transcript_with_bearer(
    ...
    let client_token = match required_secret(env, TRANSCRIPT_ANALYSIS_CLIENT_TOKEN) {
...
async fn analyze_validated_request(
    ...
    // Async-capable clients always get the fingerprint-keyed job: every HTTP
    // exchange stays short, a lost response is recovered by polling (or
    // re-submitting) the same fingerprint, and completed results serve
    // idempotently until their TTL purge. The inline path below survives
    // only for legacy unbilled `async_supported: false` callers, whose
    // single exchange spans the whole model call.
    ...
    // resolve_billing already refused billed sync requests; unreachable
    // backstop so a future caller can never bill without a durable record.
    if billing.is_some() {
...
async fn handle_install_delete(req: &mut Request, env: &Env) -> Result<Response> {
    ...
    // via their cleanup alarms, so there is nothing to purge there. The
    // 30-minute job DO records purge on their TTL alarms; a future shared
    // store, when it lands, holds nothing user-linked by design
    // needs a separate durable store.
    storage::delete_install_rows(&db, &authenticated.install_id).await?;

    json_success(200, &serde_json::json!({ "message": "deleted" }))
}

/// Account bootstrap: App Attest envelope only — the
/// bearer probe lane never bills and carries no install identity. Served
/// whenever a credit backend is reachable, independent of BILLING_REQUIRED,
/// so installs can link BEFORE a lane's billing flips on (no flip-day race).
async fn handle_account_bootstrap(req: &mut Request, env: &Env) -> Result<Response> {
    let db = match required_d1(env, TRANSCRIPT_ANALYSIS_DB) {
        Ok(db) => db,
        Err(error) => return json_error(503, error),
    };
    ...
    let authenticated = match authenticate_envelope(
        ...
        now_seconds(),
        "POST",
        ACCOUNT_BOOTSTRAP_PATH,
        MAX_ACCOUNT_BOOTSTRAP_BODY_BYTES,
        MAX_ACCOUNT_BOOTSTRAP_BODY_BYTES,
    )
    .await?
    {
    ...
    let request = match serde_json::from_str::<BootstrapRequest>(&authenticated.payload) {
        Ok(request) => request,
        Err(_) => return json_error_code(400, "invalid_json"),
    };
    if request.schema_version != SCHEMA_VERSION {
        return json_error_code(400, "invalid_bootstrap_request");
    }

    match config.credit_backend {
        CreditBackend::Dev => {
            // Development fake: install-keyed accounts, fixed grant, no
            // Apple identity involved. The claim is first-writer-wins so
            // concurrent bootstraps of one install converge on one account.
            let now = now_seconds();
            let account_id = match account_for_install(&db, &authenticated.install_id).await? {
                ...
                None => {
                    ...
                    claim_install_account(&db, &authenticated.install_id, &candidate, now).await?
                }
            };
            let credit = DevCreditAuthority::new(
                env.d1(TRANSCRIPT_ANALYSIS_DB)?,
                config.dev_credit_grant_seconds,
            );
            match credit.bootstrap(&account_id, now).await {
                Ok(balance) => json_success(
                    200,
                    &BootstrapResponse {
                ...
                Err(error) => json_error(
                    503,
                    ErrorResponse::with_detail(ERROR_BILLING_UNAVAILABLE, error.code()),
                ),
            }
        }
        CreditBackend::Purchase => {
            // Identity: the verified AppTransaction JWS rides inside the
            // authenticated envelope; PurchaseWorker verifies it before any
            // account exists. Same Apple identity converges on the same
            // account RTW uses (HMAC of the AppTransaction, not the
            // install).
            ...
            // Every fail-closed arm below logs its cause: a bare 503 with
            // nothing in observability turns flip-day misconfiguration into
            // a bisection exercise (content-free: codes and messages only).
            let purchase = match PurchaseAuthority::from_env(env) {
                Ok(purchase) => purchase,
                Err(error) => {
                    worker::console_error!(
                        "transcript-analysis bootstrap purchase authority unavailable: {}",
                        error.code()
                    );
            ...
            let upstream_body = serde_json::json!({
                "schema_version": SCHEMA_VERSION,
                "install_key": purchase_install_key(&authenticated.install_id),
                "app_transaction_jws": jws,
            });
            let mut upstream = match purchase
                ...
                .await
            {
                ...
                Err(error) => {
                    worker::console_error!(
                        "transcript-analysis bootstrap purchase call failed: {error}"
                    );
                    return json_error_code(503, ERROR_BILLING_UNAVAILABLE);
                }
            };
            ...
            let parsed: PurchaseBootstrapUpstream = match upstream.json().await {
                ...
                Err(_) => {
                    worker::console_error!(
                        "transcript-analysis bootstrap purchase response decode failed"
                    );
                    return json_error_code(503, ERROR_BILLING_UNAVAILABLE);
                }
            };
...
/// Mirror an upstream JSON response (status + body) without interpreting it.
async fn mirror_json_response(response: &mut Response) -> Result<Response> {
    ...
    Ok(Response::builder()
        ...
        .fixed(text.into_bytes()))
}

pub(crate) fn mint_billing_id() -> Result<String> {
    Ok(billing_id_from_token(
        &random::random_urlsafe_token(12)
            .map_err(|error| worker::Error::RustError(error.to_string()))?,
    ))
}

/// Resolve the billing directive for an authenticated App Attest install.
/// `Ok(None)` = billing dark; `Err(response)` = typed refusal to send now.
async fn resolve_billing(
    db: &worker::D1Database,
    config: &AppConfig,
    install_id: &str,
    request: &crate::types::TranscriptAnalysisRequest,
) -> Result<std::result::Result<Option<BillingContext>, Response>> {
    if !config.billing_required {
        return Ok(Ok(None));
    }
    // Billed work runs only on the durable job lane: the sync exchange has
    // no record to repair from, so a client disconnect between reserve and
    // settle would strand the hold forever (PurchaseWorker has no
    // reservation expiry). The shipped app always requests async.
    if !request.async_supported {
        return Ok(Err(json_error_code(400, ERROR_ASYNC_REQUIRED)?));
    }
    let account_id = match account_for_install(db, install_id).await {
        ...
        Err(error) => {
            ...
            return Ok(Err(json_error_code(503, ERROR_BILLING_UNAVAILABLE)?));
        }
    };
    let duration = authoritative_duration_seconds(request);
    let Some(charge_seconds) = charge_seconds_for_duration(duration) else {
        // Validation already rejects non-finite durations; defense in depth.
        return Ok(Err(json_error_code(400, "invalid_duration")?));
    };
    Ok(Ok(Some(BillingContext {
        account_id,
        charge_seconds,
    })))
}

/// Shared typed mapping for a refused reserve: insufficient
/// funds becomes the client-facing 402 with a best-effort balance snapshot;
/// an account the backend does not know maps to `bootstrap_required` (a
/// stale install link — e.g. minted against a different backend — repairs
/// itself through a re-bootstrap); every other refusal fails CLOSED behind
/// `billing_unavailable` (internal PurchaseWorker codes never leak to app
/// clients).
pub(crate) async fn reserve_failure_response(
    credit: &CreditAuthority,
    ...
    match error {
        CreditError::Insufficient => {
            let balance = credit
                .balance(&context.account_id, now_seconds())
                .await
                .ok();
            json_success(
                402,
                &InsufficientSecondsResponse {
                    error: ERROR_INSUFFICIENT_SECONDS.to_string(),
                    charge_seconds: context.charge_seconds,
                    balance,
                },
            )
        }
        CreditError::AccountNotFound => {
            worker::console_error!(
                "transcript-analysis reserve refused: linked account unknown to backend"
            );
            json_error_code(403, ERROR_BOOTSTRAP_REQUIRED)
        }
        other => {
            // Content-free: stable code only.
            worker::console_error!("transcript-analysis reserve failed: {}", other.code());
            json_error_code(503, ERROR_BILLING_UNAVAILABLE)
        }
    }
}
