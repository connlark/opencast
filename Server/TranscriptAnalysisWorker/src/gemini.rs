...
pub enum GeminiParseError {
    ...
    MalformedModelJson,
    /// A MAX_TOKENS finish is a hard parse failure here, not a warning: a
    /// truncated schema-constrained response can still be a syntactically
    /// valid JSON prefix, and evaluation measured thinking blowouts as transient
    /// (1 of 3 sn1085 runs blew to 27.9k thinking tokens and truncated; the
    /// immediate same-config retry produced the canonical output). The
    /// caller's single full re-request is the fix.
    MaxTokensTruncated,
    /// Any other non-STOP finish (SAFETY and friends): never trust the text.
    NonStopFinishReason,
}
...
impl GeminiParseError {
    pub fn code(&self) -> &'static str {
        match self {
            ...
            GeminiParseError::MaxTokensTruncated => "max_tokens_truncated",
            GeminiParseError::NonStopFinishReason => "non_stop_finish_reason",
        }
    }
}

/// Parse result for one Gemini response. Malformed or truncated model output
/// is not an error path: `output` is `None`, the failure code and any
/// finish-reason warning ride along, and the caller decides between the one
/// full re-request and failing the run. Transport-level failures never reach
/// this type.
#[derive(Debug, Clone)]
...
pub fn parse_generate_content_response(body: &str) -> ParsedModelResponse {
    ...
    if finish_reason.is_some_and(|reason| reason != "STOP") {
        let failure = if finish_reason == Some("MAX_TOKENS") {
            GeminiParseError::MaxTokensTruncated
        } else {
            GeminiParseError::NonStopFinishReason
        };
        return ParsedModelResponse {
            output: None,
            usage,
