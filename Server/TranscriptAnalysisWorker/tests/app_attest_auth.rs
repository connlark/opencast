//! Exercises the shared `opencast-app-attest-core` crate against a REAL
//! captured device attestation plus this worker's migration schema. The
//! pinned fixture was captured against the ad-analysis worker (its signed
//! client data binds `/v1/ad-analysis/transcript`) — that path is fixture
//! DATA here, not a route: what these tests pin is core attestation/
//! assertion verification, counter CAS replay rejection, and the SEC1
//! public-key bytes the workerd integration tests seed into local D1. This
//! worker's own path binding is covered by the derived-pin poll test below
//! and by the integration suite's synthetic identities.
...
/// Pins the client-data derivation for this worker's poll path
/// (`POST\n{path}\n{sha256(payload)}`), independently computed (Python
/// hashlib) so Rust and the Swift client cannot drift without tripping it.
#[test]
fn poll_request_binding_matches_derived_pin() {
    let job_id = "fingerprint-123";
    let path = format!("/v1/transcript-analysis/jobs/{job_id}");
    let payload = format!(r#"{{"job_id":"{job_id}"}}"#);

    assert_eq!(
        hex::encode(request_client_data_hash("POST", &path, &payload)),
        "db5ee85d1f136dc962b61c61f349c2d4dc3def6c09663f0575f37e159b8f55aa"
    );
}
